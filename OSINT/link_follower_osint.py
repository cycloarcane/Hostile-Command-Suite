...
def _cli() -> None:
    ...
    p = argparse.ArgumentParser(description="Web page content fetcher and parser")
    sub = p.add_subparsers(dest="cmd", required=True)

    # fetch command
    fetch = sub.add_parser("fetch", help="Fetch a single URL")
    fetch.add_argument("url", help="URL to fetch")
    fetch.add_argument("--delay", type=float, default=3.0)
    fetch.add_argument("--timeout", type=int, default=30)
    fetch.add_argument("--max-content-length", type=int, default=1000000)
    fetch.add_argument("--no-text", action="store_false", dest="extract_text")
    fetch.add_argument("--no-links", action="store_false", dest="extract_links")
    fetch.add_argument("--no-metadata", action="store_false", dest="extract_metadata")

    # fetch-multiple command
    fetch_multi = sub.add_parser("fetch-multiple", help="Fetch multiple URLs")
    fetch_multi.add_argument("urls", nargs="+", help="URLs to fetch")
    fetch_multi.add_argument("--max-urls", type=int, default=10)
    fetch_multi.add_argument("--delay", type=float, default=3.0)
    fetch_multi.add_argument("--timeout", type=int, default=30)
    fetch_multi.add_argument("--max-content-length", type=int, default=1000000)
    fetch_multi.add_argument("--no-text", action="store_false", dest="extract_text")
    fetch_multi.add_argument("--no-links", action="store_false", dest="extract_links")
    fetch_multi.add_argument("--no-metadata", action="store_false", dest="extract_metadata")
    
    # check command
    sub.add_parser("check", help="Check if the tool is working")

    args = p.parse_args()
    
    if args.cmd == "check":
        out = {"status": "ok", "message": "link_follower_osint ready"}
    elif args.cmd == "fetch":
        out = fetch_url(
            url=args.url,
            delay=args.delay,
    ...
    elif args.cmd == "fetch-multiple":
        out = fetch_multiple_urls(
            urls=args.urls,
...
if __name__ == "__main__":
    if len(sys.argv) > 1 and sys.argv[1] in {"fetch", "fetch-multiple", "check"}:
        _cli()
    else:
